...
``` r
df = tibble(document = 0, word = 0) # Initialize
for (j in 1:4){
...
We can see that all words that came from Storyline-3 were also correctly
classified to that storyline. Words from Storyline-1 and Storyline-4
were mainly correctly classified, but carried some minsinterpretation as
well. Storyline-2 was equally split between the first and second
storyline.
